...
def main():
    ...
    if args.clip_path:
        ...
        tiles = gpd.read_file(r"TILES\TILES.shp").to_crs("EPSG:4326")
    ...
    tiempo_segundos = (time.time() - ahora) % 60
    print(f"Tiempo de ejecución: {int(tiempo_horas)}:{int(tiempo_minutos)}:{int(tiempo_segundos)} horas ")
    # subprocess.run(["python3.6", r"scripts/PROCESAR/STack_stacks.py", outdir, f"stack_{tile}_{index_name}_{res}.dat"])
