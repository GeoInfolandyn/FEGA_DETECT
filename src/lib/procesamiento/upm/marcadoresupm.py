...
def calcular_marcadores(dataframe:pd.DataFrame) -> pd.DataFrame:
    ...
    def load_umbrals():
        ...
        try:
            umbrals = pd.read_csv('data/umbrals/umbrals.csv')
...
def parametrizar(dataframe:pd.DataFrame) -> None:
    ...
    def gaussianfilter(data, sigma = 2, title = None):
        ...
        plt.savefig(f'data/umbrals/{title}.png')
        plt.close()
        return mean, std, n       
    ...
    os.makedirs('data/umbrals', exist_ok=True)
    ...
    umbrals.to_csv('data/umbrals/umbrals.csv', index=False)
